...
int main()
{
  ...
  //OTime = make_vector(OTime,NFFT-1);
	double *OTime;
	OTime = dvector(1,NFFT-1);
  for(int i=0;i<NFFT-1;i++)
    {
		OTime[i] = ODT*i;
      printf("OTime[%d] = %4.4f\n",i,OTime[i]);
    }
	free_dvector(OTime,1,NFFT-1);
  double DF=1.0;
  ...
  //Area = make_vector(Area,DSNum);
	Area = dvector(1,DSNum);
  //UnitNormal = make_dmatrix(DSNum,3);
	UnitNormal = dmatrix(1,DSNum,1,3);
  ...
  double complex ****PBD1,****PBD2,****PBD3; 
	PBD1 = d4dmatrix(1,FNum, 1, OSNum, 1, BNum, 1, DSNum);
	PBD2 = d4dmatrix(1,FNum, 1, OSNum, 1, BNum, 1, DSNum);
	PBD3 = d4dmatrix(1,FNum, 1, OSNum, 1, BNum, 1, DSNum);

	
	double complex ***PB1,***PB2,***PB3;
	PB1 = d3dmatrix(1,FNum, 1, OSNum, 1, BNum);
	PB2 = d3dmatrix(1,FNum, 1, OSNum, 1, BNum);
	PB3 = d3dmatrix(1,FNum, 1, OSNum, 1, BNum);
	
	double complex **P1,**P2,**P3;
	P1 = dmatrix(1,FNum, 1, OSNum);
	P2 = dmatrix(1,FNum, 1, OSNum);
	P3 = dmatrix(1,FNum, 1, OSNum);

	
	double complex **pF;
	pF = dmatrix(1,FNum, 1, OSNum);

	double  **Op, *OF;
	Op = dmatrix(1,FNum, 1, OSNum);
	OF = dvector(1,FNum);

  FILE *fwritepF,*fwriteSP1,*fpPB1,*fpPB2,*fwrite1;
  ...
  //for(int n =0; n< 2;n++)
  {
	  ...
	  ka = Omega/C_0;
		OF[n] = (OmegaM+BNum*n*OmegaR)/(2*PI);
	  ...
	  for(int m=20;m<21;m++)//OSNum=
	  {
		  ...
		  pF[n][m] = (P1[n][m]+P2[n][m]+P3[n][m])/(4*PI*Tint);
			fprintf(fwritepF,"%12.10f	%12.10f\n",creal(pF[n][m]),cimag(pF[n][m]));
			
			Op[n][m] = 2*sqrt(pow(creal(pF[n][m]),2)+pow(cimag(pF[n][m]),2));
	...
	fclose(fwritepF);fclose(fwriteSP1);fclose(fpPB1);fclose(fpPB2);fclose(fwrite1);
  ...
  free(ZO); 
	...
	free_dvector(Area,1,DSNum);
	free_dmatrix(UnitNormal,1,DSNum,1,3);
	
	free_d4dmatrix(PBD1,1,FNum, 1, OSNum, 1, BNum, 1, DSNum);
	free_d4dmatrix(PBD2,1,FNum, 1, OSNum, 1, BNum, 1, DSNum);
	free_d4dmatrix(PBD3,1,FNum, 1, OSNum, 1, BNum, 1, DSNum);
	
	free_d3dmatrix(PBD1,1,FNum, 1, OSNum, 1, BNum);
	free_d3dmatrix(PBD2,1,FNum, 1, OSNum, 1, BNum);
	free_d3dmatrix(PBD3,1,FNum, 1, OSNum, 1, BNum);
	
	free_dmatrix(P1, 1,FNum, 1, OSNum);
	free_dmatrix(P2, 1,FNum, 1, OSNum);
	free_dmatrix(P3, 1,FNum, 1, OSNum);
	
	free_dmatrix(pF, 1,FNum, 1, OSNum);
	free_dmatrix(Op, 1,FNum, 1, OSNum);
	free_dvector(OF,1,FNum);
